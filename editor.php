<?php
    ...
    $sql_cmd = "SELECT 
        ...
        C.classify_name AS classify
        FROM articles AS A LEFT JOIN classifies AS C 
...
<html lang="en">
  ...
  <body> 
    ...
    <section>
      <div class="container">
        <div class="row">
          <div class="add_articles col-md-9 pb-3">
            <h2>新增文章</h2>
            <form action="handle_edit_article.php" method="post">
              ...
              <input list="article_classify_options" name="article_classify" id="article_classify" class="form-control" value="<?php echo $classify_name; ?>" required>
              <datalist id="article_classify_options">
                <?php foreach($classify_list as $classify) { ?>
                  <option 
                    value="<?php echo $classify['classify_name'] ?>" 
                  >
                <?php } ?>  
              </datalist>
              <label for="editor"class="form-label">文章內容</label>
              <textarea name="article_content" id="editor" class="form-control mb-2" cols="30" rows="10"><?php echo $content ?></textarea>
              <div class="add_btn d-flex justify-content-end align-items-center">
                <button type="submit" class="add btn">修改文章</button>
              </div>
